...
当前 CodeBuddy IDE 概述文档未展示专门的 Hook 配置入口。接入 CodeBuddy 时，不应假设固定 Hook 配置格式；建议优先通过 MCP、自定义斜杠指令、Rules、Skills 或自定义 Agent 工作流触发 `capfind agent`。

The current CodeBuddy IDE overview does not show a dedicated hook configuration entry. Do not assume a fixed CodeBuddy hook format; prefer MCP, custom slash commands, Rules, Skills, or custom Agent workflows to trigger `capfind agent`.

### Claude Code / Cursor note

不同 Agent 产品的 Hook 配置格式可能变化。推荐把 `scripts/capfind-agent-hook.sh` 作为唯一稳定入口，产品侧只负责在“准备新增能力”前传入一段简短 task 文本。
