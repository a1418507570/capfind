...
pub(crate) const MAX_JAR_BYTES: u64 = 64 * 1024 * 1024;
...
pub fn scan_and_parse(repo_root: &Path) -> Result<Vec<Capability>> {
    scan_and_parse_with_config(repo_root, &IndexConfig::default())
}

pub fn scan_and_parse_with_config(
    repo_root: &Path,
    index_config: &IndexConfig,
) -> Result<Vec<Capability>> {
    let units = scan_source_units(repo_root, index_config)?;
...
fn scan_source_units(repo_root: &Path, index_config: &IndexConfig) -> Result<Vec<SourceUnit>> {
    use ignore::WalkBuilder;

    let repo_root = repo_root
        .canonicalize()
        .with_context(|| format!("failed to resolve repo root {}", repo_root.display()))?;
    let scan_roots = resolve_scan_roots(&repo_root, &index_config.roots)?;
    let include_set = Arc::new(build_include_set(&index_config.include)?);
        ...
        handles.push(std::thread::spawn(move || {
            ...
            walker.run(|| {
                ...
                Box::new(move |entry| {
                    ...
                    if is_capfind_ignored(capfind_ignore.as_ref().as_ref(), path, is_dir) {
                        return if is_dir {
                    ...
                    let rel_path_buf = path.strip_prefix(&root).unwrap_or(path).to_path_buf();
                    if !matches_include(include_set.as_ref().as_ref(), &rel_path_buf)
    ...
    let mut units = rx.into_iter().collect::<Vec<_>>();
    for handle in handles {
        if handle.join().is_err() {
            bail!("source scanner thread panicked");
        }
    }
    units.sort_by(|a, b| a.rel_path.cmp(&b.rel_path));
    let mut seen = HashSet::new();
    units.retain(|unit| seen.insert(unit.rel_path.clone()));
    Ok(units)
}

pub(crate) fn resolve_scan_roots(repo_root: &Path, roots: &[String]) -> Result<Vec<PathBuf>> {
    ...
    for root in requested_roots {
        ...
        let canonical = candidate
            .canonicalize()
            .with_context(|| format!("index root not found: {root}"))?;
        if !canonical.starts_with(repo_root) {
            bail!(
                "index root {} resolves outside repo root {}",
                root,
                repo_root.display()
            );
        }
        if !canonical.is_dir() {
            bail!("index root is not a directory: {root}");
        }
    ...
    if resolved.is_empty() {
        resolved.push(repo_root.to_path_buf());
    }
    Ok(resolved)
}

pub(crate) fn build_include_set(patterns: &[String]) -> Result<Option<GlobSet>> {
    ...
    for pattern in patterns.iter().map(|pattern| pattern.trim()) {
        ...
        builder.add(Glob::new(pattern).with_context(|| {
            format!("invalid index.include pattern in .capfind/config.toml: {pattern}")
        })?);
        has_patterns = true;
    }
    if has_patterns {
...
pub(crate) fn matches_include(include_set: Option<&GlobSet>, rel_path: &Path) -> bool {
    include_set
        .map(|set| set.is_match(rel_path))
        .unwrap_or(true)
}

pub(crate) fn is_supported_index_input(path: &Path) -> bool {
    let ext = path.extension().and_then(|e| e.to_str()).unwrap_or("");
    let file_name = path.file_name().and_then(|n| n.to_str()).unwrap_or("");
    matches!(ext, "java" | "go" | "proto" | "xml" | "jar")
        || matches!(file_name, "pom.xml" | "build.gradle" | "build.gradle.kts")
}

pub(crate) fn is_skipped_source_path(path: &Path) -> bool {
...
pub(crate) fn file_stat(path: &Path, content: &str) -> Option<FileStat> {
...
fn collect_project_packages(pairs: &[(String, String)]) -> HashSet<String> {
    pairs
        .iter()
...
fn is_external_import(symbol: &str, project_packages: &HashSet<String>) -> bool {
    if symbol.starts_with("java.") {
        return false;
    }
    let mut candidate = symbol.trim_end_matches(".*");
    loop {
        if project_packages.contains(candidate) {
            return false;
        }
...
fn jar_method_capability(rel_path: &str, method: JarMethodEntry<'_>) -> Capability {
    ...
    if !method.access.is_empty() {
        annotations.push(format!("access:{}", method.access));
    }
    if method.is_static {
        annotations.push("static".to_string());
    }
    if method.is_deprecated {
        annotations.push("deprecated".to_string());
    }
    if !method.throws.is_empty() {
        annotations.push(format!("throws:{}", method.throws.join(",")));
    }
    if let Some(generic_signature) = method.generic_signature {
        annotations.push(format!("generic:{generic_signature}"));
    }
    external_capability(
        rel_path,
        1,
...
struct JarMethodEntry<'a> {
    ...
    is_deprecated: bool,
    throws: Vec<&'a str>,
    generic_signature: Option<&'a str>,
}
    ...
    fn parse(line: &'a str) -> Option<Self> {
        ...
        for part in parts {
            if part == "static" {
                ...
                is_deprecated = true;
            } else if let Some(value) = part.strip_prefix("access:") {
                access = value;
            } else if let Some(value) = part.strip_prefix("throws:") {
                throws = value.split(',').filter(|value| !value.is_empty()).collect();
            } else if let Some(value) = part.strip_prefix("generic:") {
                generic_signature = Some(value);
            }
        }
...
fn jar_methods_from_class_bytes(class_name: &str, bytes: &[u8]) -> Vec<String> {
    ...
    methods
        ...
        .map(|method| {
            let params = method.params.join(",");
            let mut flags = vec![format!("access:{}", method.access)];
            if method.is_static {
            ...
            if !method.throws.is_empty() {
                flags.push(format!("throws:{}", method.throws.join(",")));
            }
            if let Some(ref generic_signature) = method.generic_signature {
                flags.push(format!("generic:{generic_signature}"));
            }
            format!(
                "method\t{}\t{}\t{}\t{}\t{}",
                class_name,
...
fn parse_class_methods(bytes: &[u8]) -> Option<Vec<ParsedClassMethod>> {
    ...
    while i < cp_count {
        ...
        match tag {
            ...
            7 => {
                ...
                cp[i] = Some(ConstantPoolEntry::Class(name_index));
            }
    ...
    for _ in 0..method_count {
        ...
        let attrs = read_method_attributes(&mut reader, &cp)?;
        ...
        methods.push(ParsedClassMethod {
            ...
            params,
            return_type,
            access: method_access(access),
            is_static: access & 0x0008 != 0,
            is_deprecated: attrs.is_deprecated,
            throws: attrs.throws,
            generic_signature: attrs.generic_signature,
        });
    }
...
fn method_access(access: u16) -> &'static str {
    if access & 0x0001 != 0 {
        "public"
...
fn read_method_attributes(
    ...
    cp: &[Option<ConstantPoolEntry>],
) -> Option<MethodAttributes> {
    let count = reader.read_u2()? as usize;
    ...
    for _ in 0..count {
        ...
        match cp_utf8(cp, name_index).unwrap_or_default() {
            "Deprecated" => {
                attrs.is_deprecated = true;
            }
            "Exceptions" => {
                let exception_count = reader.read_u2()? as usize;
                for _ in 0..exception_count {
                    let class_index = reader.read_u2()? as usize;
                    if let Some(class_name) = cp_class_name(cp, class_index) {
                        attrs.throws.push(class_name.replace('/', "."));
                    }
                }
            }
            "Signature" => {
                let signature_index = reader.read_u2()? as usize;
                attrs.generic_signature = cp_utf8(cp, signature_index).map(str::to_string);
            }
            _ => {}
        }
        let consumed = reader.pos.checked_sub(start)?;
        if consumed > len {
            return None;
        }
...
fn cp_class_name(cp: &[Option<ConstantPoolEntry>], index: usize) -> Option<&str> {
    let name_index = match cp.get(index)? {
...
pub(crate) fn load_capfindignore(repo_root: &Path) -> Result<Option<Gitignore>> {
...
pub(crate) fn is_capfind_ignored(ignore: Option<&Gitignore>, path: &Path, is_dir: bool) -> bool {
    ignore
...
"#;
    ...
    #[test]
    fn capfindignore_excludes_go_glob_before_parsing() {
        let dir = tempfile::tempdir().unwrap();
        let root = dir.path();
        fs::write(root.join(".capfindignore"), "**/*.go\n").unwrap();
        fs::write(
            root.join("main.go"),
            r#"
package main
...
func routes(router *gin.Engine) {
    router.GET("/ignored", handler) // 接口描述接口描述接口描述接口描述接口描述接口描述接口描述
}
"#,
        )
        .unwrap();
        ...
        let caps = scan_and_parse(root).unwrap();
        assert!(caps.is_empty(), "ignored Go source should not be parsed");
    }

    #[test]
    fn index_roots_limit_scan_scope() {
        let dir = tempfile::tempdir().unwrap();
        let root = dir.path();
        fs::create_dir_all(root.join("small")).unwrap();
        fs::create_dir_all(root.join("large")).unwrap();
        fs::write(root.join("small/IncludedController.java"), CONTROLLER).unwrap();
        fs::write(root.join("large/OutsideController.java"), CONTROLLER).unwrap();

        let cfg = IndexConfig {
            roots: vec!["small".to_string()],
            include: Vec::new(),
        };
        let caps = scan_and_parse_with_config(root, &cfg).unwrap();
        let endpoints = caps
            .iter()
            .filter(|cap| cap.kind == Kind::HttpEndpoint)
            .collect::<Vec<_>>();
        ...
        assert_eq!(endpoints.len(), 1);
        assert_eq!(endpoints[0].file, "small/IncludedController.java");
    }

    #[test]
    fn index_include_limits_supported_inputs() {
        let dir = tempfile::tempdir().unwrap();
        let root = dir.path();
        fs::create_dir_all(root.join("src")).unwrap();
        fs::write(root.join("src/IncludedController.java"), CONTROLLER).unwrap();
        fs::write(
            root.join("src/routes.go"),
            r#"
package main

func routes(router *gin.Engine) {
    router.GET("/go", handler)
}
"#,
        )
        .unwrap();
        ...
        let cfg = IndexConfig {
            ...
            include: vec!["**/*.java".to_string()],
        };
        let caps = scan_and_parse_with_config(root, &cfg).unwrap();
        let files = caps.iter().map(|cap| cap.file.as_str()).collect::<Vec<_>>();

        assert!(files
            .iter()
            .any(|file| *file == "src/IncludedController.java"));
        assert!(!files.iter().any(|file| *file == "src/routes.go"));
    }
    ...
    fn deflated_jar_with_classes(entries: &[(&str, Vec<u8>)]) -> Vec<u8> {
...
"#,
        ...
        .unwrap();
        fs::write(
            root.join("libs/vendor-compressed-1.0.jar"),
            deflated_jar_with_classes(&[(
                "com/vendor/CompressedApi.class",
                fake_class_with_methods(&[(
                    "lookup",
...
"#
        ...
        assert!(signatures.contains(&"local jar vendor-compressed-1.0.jar"));
        assert!(signatures.contains(&"jar class com.vendor.CompressedApi"));
        assert!(signatures.contains(
            &"jar method com.vendor.CompressedApi#lookup(java.lang.String): java.lang.String"
        ));
        assert!(caps.iter().any(|cap| {
            cap.signature
                == "jar method com.vendor.ClientApi#query(com.vendor.QueryRequest, int): com.vendor.QueryResponse"
                && cap.annotations.iter().any(|a| a == "access:public")
        }));
        assert!(signatures.contains(
