...
fn has_methods_suffix(src: &str, offset: usize) -> bool {
    let start = floor_char_boundary(src, offset);
    let end = floor_char_boundary(src, offset.saturating_add(128));
...
fn byte_offset_to_line(src: &str, byte_offset: usize) -> u32 {
    let end = floor_char_boundary(src, byte_offset);
    src[..end].matches('\n').count() as u32 + 1
}

fn floor_char_boundary(src: &str, offset: usize) -> usize {
...
"#;
    ...
    #[test]
    fn parses_routes_when_cjk_text_is_near_suffix_probe() {
        let src = r#"
package main

// 这是一个接口描述
func routes(router *gin.Engine) {
    router.GET("/hello", handler) // 接口描述接口描述接口描述接口描述接口描述接口描述接口描述
}
"#;
        let caps = parse_go_file(Path::new("main.go"), src);
        assert_eq!(caps.len(), 1);
        assert_eq!(caps[0].http.as_ref().unwrap().path, "/hello");
    }
}
